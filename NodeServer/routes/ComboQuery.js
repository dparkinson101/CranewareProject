...
router.get('/historicdata', function(req, res, next){
    ...
    var query = '(select distinct averageCoveredCharges, averageMedicarePayments, averageTotalPayments, 2017 as years from financial2017 where dRGDefinition = ' + codes + ' and providerId = ' + providerIds + ' limit 1 ) union all (select distinct averageCoveredCharges, averageMedicarePayments, averageTotalPayments, 2016 as years from financial2016 where dRGDefinition = ' + codes + ' and providerId = ' + providerIds + ' limit 1) union all (select distinct averageCoveredCharges, averageMedicarePayments, averageTotalPayments, 2015 as years from financial2015 where dRGDefinition = ' + codes + ' and providerId = ' + providerIds + ' limit 1) union all (select distinct averageCoveredCharges, averageMedicarePayments, averageTotalPayments, 2014 as years from financial2014 where dRGDefinition = ' + codes + ' and providerId = ' + providerIds + ' limit 1) union all(select distinct averageCoveredCharges, averageMedicarePayments, averageTotalPayments, 2013 as years from financial2013 where dRGDefinition = ' + codes + ' and providerId = ' + providerIds + ' limit 1) union all (select distinct averageCoveredCharges, averageMedicarePayments, averageTotalPayments, 2012 as years from financial2012 where dRGDefinition = ' + codes + ' and providerId = ' + providerIds + ' limit 1) union all (select distinct averageCoveredCharges, averageMedicarePayments, averageTotalPayments, 2011 as years from financial2011 where dRGDefinition = ' + codes + ' and providerId = ' + providerIds + ' limit 1)';
    connection.query(query, function(err, results){
        if(err){
            console.log(err);
            res.status(500).json({ "status_code": 500, "status_message": "internal server error" + err });
